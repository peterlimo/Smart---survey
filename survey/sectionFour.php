...
if(!$_COOKIE["respondent_id"]) {
 ...
 $quiz = "select * from quiz where section_id = 4 group by group_id";
        ...
        <div class="row">
            ...
            while($row = mysqli_fetch_array($qq))
            {
                ...
                if($row['group_type'] =="likert" && $row['section_id'] == $my_section_id)
                {
                    ...
                    $getQ ="select * from quiz where group_id =".$row['group_id']." and section_id=4";
            ...
            ?>
                <input type="submit" class="btn btn-outline-primary mt-2 mb-4" style="float: right;width:100%" name="submit"></input>
            </form>
            <?php
        
                $sqlPrevious = "select * from sections where section_id=4";
