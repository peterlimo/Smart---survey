...
if(!$_COOKIE["respondent_id"]) {
 ...
 $quiz = "select * from quiz where section_id = 1 group by group_id";
   ...
   <?php
    ...
    if (isset($_POST['submit'])) {
        ...
        while($row = mysqli_fetch_array($qq))
            {
                $getQuiz ="select * from quiz where group_id =".$row['group_id']." and section_id=1";
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            while($row = mysqli_fetch_array($qq))
            {
                ...
                else{ if($row['group_title']!="NA" && $row['section_id'] == $my_section_id ){
                        echo '<div class="mt-2"><h5 style="color:#202124;font-weight: 400;font-family:Roboto">'.$row['group_title'].'</h5></div>';
                        }
                $getQ ="select * from quiz where group_id =".$row['group_id']." and section_id=1";
                $gg = mysqli_query($conn,$getQ);
